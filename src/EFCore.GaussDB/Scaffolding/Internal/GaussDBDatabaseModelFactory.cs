...
/// <summary>
///     The default database model factory for GaussDB.
/// </summary>
public class GaussDBDatabaseModelFactory : DatabaseModelFactory
{
    #region Fields
    ...
    /// <summary>
    ///     Constructs an instance of the <see cref="GaussDBDatabaseModelFactory" /> class.
    /// </summary>
    public GaussDBDatabaseModelFactory(IDiagnosticsLogger<DbLoggerCategory.Scaffolding> logger)
    {
    ...
    public override DatabaseModel Create(DbConnection dbConnection, DatabaseModelFactoryOptions options)
    {
        ...
        var connection = (GaussDBConnection)dbConnection;
    ...
    private static void PopulateGlobalDatabaseInfo(GaussDBConnection connection, DatabaseModel databaseModel)
    {
...
""";
        using var command = new GaussDBCommand(commandText, connection);
        ...
        using (var reader = command.ExecuteReader())
        {
            while (reader.Read())
            {
                ...
                foreach (var storageParameter in storageParameters)
                {
                    ...
                    {
                        table[GaussDBAnnotationNames.StorageParameterPrefix + paramName] = paramValue;
                    }
                }
...
""";
        ...
        foreach (var tableGroup in tableGroups)
        {
            ...
            foreach (var record in tableGroup)
            {
                ...
                {
                    case "a":
                        column[GaussDBAnnotationNames.ValueGenerationStrategy] = GaussDBValueGenerationStrategy.IdentityAlwaysColumn;
                        isIdentity = true;
                        break;
                    case "d":
                        column[GaussDBAnnotationNames.ValueGenerationStrategy] = GaussDBValueGenerationStrategy.IdentityByDefaultColumn;
                        isIdentity = true;
                        break;
                    default:
                        // Hacky but necessary...
                        ...
                        if (SerialTypes.Contains(systemTypeName))
                        {
                                ...
                                || column.DefaultValueSql == $"nextval('\"{column.Table.Schema}\".\"{seqName}\"'::regclass)")
                            {
                                ...
                                // Serial is the default value generation strategy, so GaussDBAnnotationCodeGenerator
                                // makes sure it isn't actually rendered
                                column[GaussDBAnnotationNames.ValueGenerationStrategy] = GaussDBValueGenerationStrategy.SerialColumn;
                            }
                        }
                ...
                if (column[GaussDBAnnotationNames.ValueGenerationStrategy] is not null)
                {
                ...
                if (isIdentity)
                {
                    ...
                    {
                        column[GaussDBAnnotationNames.IdentityOptions] = sequenceData.Serialize();
                    }
                }
                ...
                {
                    column[GaussDBAnnotationNames.CompressionMethod] = compressionMethodChar switch
                    {
                        "p" => "pglz",
                        "l" => "lz4",
                        _ => null
        ...
        IDiagnosticsLogger<DbLoggerCategory.Scaffolding> logger)
    {
        ...
        try
        {
            using var command = new GaussDBCommand("SELECT oid, opcname, opcdefault FROM pg_opclass", connection);
            using var reader = command.ExecuteReader();
        ...
        if (connection.PostgreSqlVersion >= new Version(9, 1))
        {
            using (var command = new GaussDBCommand("SELECT oid, collname FROM pg_collation", connection))
            using (var reader = command.ExecuteReader())
            {
            ...
            foreach (var tableGroup in tableGroups)
            {
                ...
                foreach (var record in tableGroup)
                {
                    ...
                    if (columnIndices.Any(i => i == 0))
                    {
                        ...
                        index[GaussDBAnnotationNames.IndexExpression] = expressions;
                        */
                    }
                    ...
                    if (columnIndices.Length > numKeyColumns)
                    {
                        ...
                        // index[GaussDBAnnotationNames.IndexInclude] = nonKeyColumns.ToArray();
                    }
                    ...
                    {
                        index[GaussDBAnnotationNames.IndexMethod] = indexMethod;
                    }
                    ...
                    {
                        index[GaussDBAnnotationNames.IndexOperators] = opClassNames;
                    }
                    ...
                    if (record.GetValueOrDefault<bool>("amcanorder"))
                    {
                        ...
                        {
                            index[GaussDBAnnotationNames.IndexNullSortOrder] = nullSortOrders;
                        }
                    }

                    if (record.GetValueOrDefault<bool>("indnullsnotdistinct"))
                    {
                        index[GaussDBAnnotationNames.NullsDistinct] = false;
                    }
                    ...
                    foreach (var storageParameter in record.GetValueOrDefault<string[]>("idx_reloptions") ?? [])
                    {
                        if (storageParameter.Split("=") is [var paramName, var paramValue])
                        {
                            index[GaussDBAnnotationNames.StorageParameterPrefix + paramName] = paramValue;
                        }
                    }
...
""";
    ...
    /// </summary>
    private static HashSet<string> GetEnums(GaussDBConnection connection, DatabaseModel databaseModel)
    {
...
""";
        ...
        while (reader.Read())
        {
            ...
            GaussDBEnum.GetOrAddPostgresEnum(databaseModel, schema, name, labels);
            enums.Add(name);
        }
    ...
    /// </summary>
    private static void GetExtensions(GaussDBConnection connection, DatabaseModel databaseModel)
    {
...
""";
        ...
        try
        {
            ...
            while (reader.Read())
            {
                ...
                GaussDBCollation.GetOrAddCollation(
