...
/// </summary>
public class GaussDBSqlTranslatingExpressionVisitor : RelationalSqlTranslatingExpressionVisitor
{
    ...
    private static readonly MethodInfo EscapeLikePatternParameterMethod =
        typeof(GaussDBSqlTranslatingExpressionVisitor).GetTypeInfo().GetDeclaredMethod(nameof(ConstructLikePatternParameter))!;
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public GaussDBSqlTranslatingExpressionVisitor(
        ...
        : base(dependencies, queryCompilationContext, queryableMethodTranslatingExpressionVisitor)
    {
        ...
        _sqlExpressionFactory = (GaussDBExpressionFactory)dependencies.SqlExpressionFactory;
        _jsonPocoTranslator = ((GaussDBMemberTranslatorProvider)Dependencies.MemberTranslatorProvider).JsonPocoTranslator;
    ...
    protected override Expression VisitUnary(UnaryExpression unaryExpression)
    {
        switch (unaryExpression.NodeType)
        {
            case ExpressionType.ArrayLength:
                ...
                // Count() which gets translated to cardinality() as usual in GaussDBQueryableMethodTranslatingExpressionVisitor.
                if (sqlOperand!.Type == typeof(byte[]) && sqlOperand.TypeMapping is GaussDBByteArrayTypeMapping or null)
                {
            ...
            // We map both IPAddress and GaussDBInet to PG inet, and translate many methods accepting GaussDBInet, so ignore casts from
            // IPAddress to GaussDBInet.
            // On the GaussDB side, cidr is also implicitly convertible to inet, and at the ADO.NET level GaussDBCidr has a similar
            // implicit conversion operator to GaussDBInet. So remove that cast as well.
            case ExpressionType.Convert
                when unaryExpression.Type == typeof(GaussDBInet)
                && (unaryExpression.Operand.Type == typeof(IPAddress) || unaryExpression.Operand.Type == typeof(Metadata.GaussDBRange)):
                return Visit(unaryExpression.Operand);
        }
    ...
    protected override Expression VisitBinary(BinaryExpression binaryExpression)
    {
        switch (binaryExpression.NodeType)
        {
                ...
                && binaryExpression.Right.Type.UnwrapNullableType().FullName == "NodaTime.LocalDate":
            {
                ...
                return GaussDBFunctionExpression.CreateWithNamedArguments(
                    "make_interval",
                    [subtraction],
                    ["days"],
                    nullable: true,
                ...
                // Note: many other date/time arithmetic operators are fully supported as-is by GaussDB - see GaussDBSqlExpressionFactory
            }

            case ExpressionType.ArrayIndex:
            {
                // During preprocessing, ArrayIndex and List[] get normalized to ElementAt; see GaussDBArrayTranslator
                Check.DebugFail(
                    "During preprocessing, ArrayIndex and List[] get normalized to ElementAt; see GaussDBArrayTranslator. "
        ...
        switch (translation)
        {
            ...
            case SqlUnaryExpression
            {
                ...
                Operand: JsonScalarExpression { TypeMapping: GaussDBOwnedJsonTypeMapping } operand
            } unary:
            {
                return unary.Update(
    ...
    protected override Expression VisitNew(NewExpression newExpression)
    {
        ...
        // This is notably done to support row value comparisons: WHERE (x, y) > (3, 4) (see e.g. GaussDBDbFunctionsExtensions.GreaterThan)
        if (newExpression.Type.IsAssignableTo(typeof(ITuple)))
        {
            return TryTranslateArguments(out var sqlArguments)
                ? new GaussDBRowValueExpression(sqlArguments, newExpression.Type)
