...
public class GaussDBTypeMappingTest
{
    #region Numeric
...
#pragma warning disable CS0618 // GaussDBCidr is obsolete, replaced by .NET IPNetwork
    [Fact]
    public void GenerateSqlLiteral_returns_cidr_literal()
        => Assert.Equal("CIDR '192.168.1.0/24'", GetMapping("cidr").GenerateSqlLiteral(new GaussDBCidr(IPAddress.Parse("192.168.1.0"), 24)));
...
#pragma warning restore CS0618
    ...
    public void GenerateSqlLiteral_returns_point_literal()
        => Assert.Equal("POINT '(3.5,4.5)'", GetMapping("point").GenerateSqlLiteral(new GaussDBPoint(3.5, 4.5)));
    ...
    public void GenerateSqlLiteral_returns_line_literal()
        => Assert.Equal("LINE '{3.5,4.5,10}'", GetMapping("line").GenerateSqlLiteral(new GaussDBLine(3.5, 4.5, 10)));
    ...
    public void GenerateSqlLiteral_returns_lseg_literal()
        => Assert.Equal("LSEG '[(3.5,4.5),(5.5,6.5)]'", GetMapping("lseg").GenerateSqlLiteral(new GaussDBLSeg(3.5, 4.5, 5.5, 6.5)));
    ...
    public void GenerateSqlLiteral_returns_box_literal()
        => Assert.Equal("BOX '((4,3),(2,1))'", GetMapping("box").GenerateSqlLiteral(new GaussDBBox(1, 2, 3, 4)));
    ...
    public void GenerateSqlLiteral_returns_circle_literal()
        => Assert.Equal("CIRCLE '<(3.5,4.5),5.5>'", GetMapping("circle").GenerateSqlLiteral(new GaussDBCircle(3.5, 4.5, 5.5)));
    ...
    {
        var mapping = new GaussDBEnumTypeMapping("dummy_enum", "dummy_enum", typeof(DummyEnum), new Dictionary<object, string>
        {
    ...
    public void GenerateSqlLiteral_returns_tid_literal()
        => Assert.Equal(@"TID '(0,1)'", GetMapping("tid").GenerateSqlLiteral(new GaussDBTid(0, 1)));

    [Fact]
    public void GenerateSqlLiteral_returns_pg_lsn_literal()
        => Assert.Equal(@"PG_LSN '12345/67890'", GetMapping("pg_lsn").GenerateSqlLiteral(GaussDBLogSequenceNumber.Parse("12345/67890")));
    ...
    {
        var source = new GaussDBPoint?[] { new GaussDBPoint(1, 1), null };

        var comparer = GetMapping(typeof(GaussDBPoint?[])).Comparer;
        var snapshot = (GaussDBPoint?[])comparer.Snapshot(source);
        Assert.Equal(source, snapshot);
        Assert.NotSame(source, snapshot);
        Assert.True(comparer.Equals(source, snapshot));
        snapshot[1] = new GaussDBPoint(2, 2);
        Assert.False(comparer.Equals(source, snapshot));
    }
    ...
    {
        var value = GaussDBRange<int>.Empty;
    ...
    {
        var value = new GaussDBRange<int>(4, false, 7, true);
        var literal = GetMapping("int4range").GenerateSqlLiteral(value);
        Assert.Equal("'(4,7]'::int4range", literal);
    }
    ...
    {
        var value = new GaussDBRange<int>(0, false, true, 7, true, false);
        var literal = GetMapping("int4range").GenerateSqlLiteral(value);
    ...
    {
        var value = new GaussDBRange<int>[]
        {
            new(4, 7),
    ...
    {
        var value = new List<GaussDBRange<int>>
        {
            new(4, 7),
            new(9, lowerBoundIsInclusive: true, 10, upperBoundIsInclusive: false),
            new(
    ...
    {
        var value = Array.Empty<GaussDBRange<int>>();
        var literal = GetMapping("int4multirange").GenerateSqlLiteral(value);
    ...
    public void GenerateCodeLiteral_returns_multirange_empty_array_literal()
    {
        var value = Array.Empty<GaussDBRange<int>>();
        var literal = CodeLiteral(value);
        Assert.Equal("new GaussDBRange<int>[0]", literal);
    }
...
#pragma warning disable CS0618 // Full-text search client-parsing is obsolete
    ...
    public void GenerateSqlLiteral_returns_tsquery_literal()
        => Assert.Equal(
            @"TSQUERY '''a'' & ''b'''",
            GetMapping("tsquery").GenerateSqlLiteral(GaussDBTsQuery.Parse("a & b")));

    [Fact]
    public void GenerateSqlLiteral_returns_tsvector_literal()
        => Assert.Equal(
            @"TSVECTOR '''a'' ''b'''",
            GetMapping("tsvector").GenerateSqlLiteral(GaussDBTsVector.Parse("a b")));
#pragma warning restore CS0618

    [Fact]
    public void GenerateSqlLiteral_returns_ranking_normalization_literal()
        => Assert.Equal(
            $"{(int)GaussDBTsRankingNormalization.DivideByLength}",
            GetMapping(typeof(GaussDBTsRankingNormalization))
                .GenerateSqlLiteral(GaussDBTsRankingNormalization.DivideByLength));
