...
public class FullTextSearchDbFunctionsGaussDBTest : IClassFixture<NorthwindQueryGaussDBFixture<NoopModelCustomizer>>
{
    ...
    public FullTextSearchDbFunctionsGaussDBTest(NorthwindQueryGaussDBFixture<NoopModelCustomizer> fixture, ITestOutputHelper testOutputHelper)
    {
        Fixture = fixture;
    ...
    public void TsVectorParse_converted_to_cast()
    {
        ...
        var tsvector = context.Customers.Select(c => GaussDBTsVector.Parse("a b")).First();
...
""");
    ...
    public void ArrayToTsVector_constants()
    {
        ...
        Assert.Equal(GaussDBTsVector.Parse("b c d").ToString(), tsvector.ToString());
        AssertSql(
            """
...
""");
    ...
    public void ArrayToTsVector_columns()
    {
        ...
        Assert.Equal(GaussDBTsVector.Parse("'Alfreds Futterkiste' 'Obere Str. 57'").ToString(), tsvector.ToString());
        AssertSql(
            """
...
""");
    ...
    {
        using var context = CreateContext();
        var tsquery = context.Customers.Select(c => GaussDBTsQuery.Parse("a & b")).First();
...
""");
    ...
    public void TsVector_Concat()
    {
        ...
        Assert.Equal(GaussDBTsVector.Parse("b:1 c:2").ToString(), tsVector.ToString());
        AssertSql(
            """
...
""");
    ...
    public void SetWeight_with_enum()
    {
        ...
        var weightedTsVector = context.Customers
            .Select(c => EF.Functions.ToTsVector("a").SetWeight(GaussDBTsVector.Lexeme.Weight.A))
            .First();
...
""");
    ...
    public void SetWeight_with_enum_and_lexemes()
    {
        using var context = CreateContext();
        var weightedTsVector = context.Customers
            .Select(c => EF.Functions.ToTsVector("a").SetWeight(GaussDBTsVector.Lexeme.Weight.A, new[] { "a" }))
            .First();
...
""");
    ...
    public void Delete_with_single_lexeme()
    {
        ...
        Assert.Equal(GaussDBTsVector.Parse("b:1").ToString(), tsVector.ToString());
        AssertSql(
            """
...
""");
    ...
    public void Filter()
    {
        using var context = CreateContext();
        var tsVector = context.Customers
            .Select(c => GaussDBTsVector.Parse("b:1A c:2B d:3C").Filter(new[] { 'B', 'C' }))
            .First();

        Assert.Equal(GaussDBTsVector.Parse("c:2B d:3C").ToString(), tsVector.ToString());
        AssertSql(
            """
...
""");
    ...
    public void ToStripped()
    {
        ...
        Assert.Equal(GaussDBTsVector.Parse("c").ToString(), strippedTsVector.ToString());
        AssertSql(
            """
        ...
        var rank = context.Customers
            .Select(
                c => EF.Functions.ToTsVector("a b c").Rank(
                    EF.Functions.ToTsQuery("b"),
                    GaussDBTsRankingNormalization.DivideByLength))
            .First();
        ...
        var headline = context.Customers
            .Where(
                c => EF.Functions.ToTsVector(c.ContactTitle)
                    .SetWeight(GaussDBTsVector.Lexeme.Weight.A)
