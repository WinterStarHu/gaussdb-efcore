...
public class GaussDBDatabaseCreatorExistsTest : GaussDBDatabaseCreatorTest
{
    [ConditionalTheory]
    ...
    {
        await using var testDatabase = GaussDBTestStore.Create("NonExisting");
    ...
    {
        await using var testDatabase = await GaussDBTestStore.GetOrCreateInitializedAsync("ExistingBlogging");
        await using var context = new BloggingContext(testDatabase);
        var creator = GetDatabaseCreator(context);
...
public class GaussDBDatabaseCreatorEnsureDeletedTest : GaussDBDatabaseCreatorTest
{
    ...
    {
        await using var testDatabase = await GaussDBTestStore.CreateInitializedAsync("EnsureDeleteBlogging");
        if (!open)
        {
...
public class GaussDBDatabaseCreatorEnsureCreatedTest : GaussDBDatabaseCreatorTest
{
        ...
        (bool CreateDatabase, bool Async, bool ambientTransaction) options)
    {
        ...
        await using var testDatabase = GaussDBTestStore.Create("EnsureCreatedTest");
        await using var context = new BloggingContext(testDatabase);
        if (createDatabase)
        {
    ...
    {
        await using var testDatabase = await GaussDBTestStore.CreateInitializedAsync("InitializedBlogging");
        await using var context = new BloggingContext(testDatabase);
...
public class GaussDBDatabaseCreatorHasTablesTest : GaussDBDatabaseCreatorTest
{
    ...
    {
        await using var testDatabase = GaussDBTestStore.GetOrCreate("NonExisting");
    ...
    {
        await using var testDatabase = await GaussDBTestStore.GetOrCreateInitializedAsync("Empty");
    ...
    {
        await using var testDatabase = await GaussDBTestStore.GetOrCreate("ExistingTables")
            .InitializeGaussDBAsync(null, t => new BloggingContext(t), null);
        var creator = GetDatabaseCreator(testDatabase);
...
public class GaussDBDatabaseCreatorDeleteTest : GaussDBDatabaseCreatorTest
{
    [ConditionalTheory]
    [InlineData(true, true)]
    [InlineData(false, false)]
    ...
    {
        await using var testDatabase = await GaussDBTestStore.CreateInitializedAsync("DeleteBlogging");
        testDatabase.CloseConnection();
    ...
    public async Task Throws_when_database_does_not_exist(bool async)
    {
        await using var testDatabase = GaussDBTestStore.GetOrCreate("NonExistingBlogging");
        var creator = GetDatabaseCreator(testDatabase);
...
public class GaussDBDatabaseCreatorCreateTablesTest : GaussDBDatabaseCreatorTest
{
    [ConditionalTheory]
    [InlineData(true, true)]
    [InlineData(false, false)]
    ...
    {
        await using var testDatabase = await GaussDBTestStore.GetOrCreateInitializedAsync("ExistingBlogging" + (async ? "Async" : ""));
        await using var context = new BloggingContext(testDatabase);
        var creator = GetDatabaseCreator(context);
...
public class GaussDBDatabaseCreatorCreateTest : GaussDBDatabaseCreatorTest
{
    [ConditionalTheory]
    [InlineData(true, false)]
    [InlineData(false, true)]
    ...
    {
        await using var testDatabase = GaussDBTestStore.GetOrCreate("CreateTest");
        var creator = GetDatabaseCreator(testDatabase);
...
[Collection("GaussDBDatabaseCreatorTest")]
public class GaussDBDatabaseCreatorTest
{
    protected static IDisposable CreateTransactionScope(bool useTransaction)
        => TestStore.CreateTransactionScope(useTransaction);

    protected static TestDatabaseCreator GetDatabaseCreator(GaussDBTestStore testStore)
    ...
    protected static IExecutionStrategy GetExecutionStrategy(GaussDBTestStore testStore)
        => new BloggingContext(testStore).GetService<IExecutionStrategyFactory>().Create();

    // ReSharper disable once ClassNeverInstantiated.Local
    private class TestGaussDBExecutionStrategyFactory(ExecutionStrategyDependencies dependencies)
        : GaussDBExecutionStrategyFactory(dependencies)
    {
    ...
    private static IServiceProvider CreateServiceProvider()
        => new ServiceCollection()
            ...
            .AddScoped<IExecutionStrategyFactory, TestGaussDBExecutionStrategyFactory>()
    ...
    {
        public BloggingContext(GaussDBTestStore testStore)
    ...
    public class TestDatabaseCreator(
        ...
        : GaussDBDatabaseCreator(dependencies, connection, rawSqlCommandBuilder, connectionLogger)
    {
