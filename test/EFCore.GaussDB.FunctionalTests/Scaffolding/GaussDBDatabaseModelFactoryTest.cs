...
public class GaussDBDatabaseModelFactoryTest : IClassFixture<GaussDBDatabaseModelFactoryTest.GaussDBDatabaseModelFixture>
{
    ...
    public GaussDBDatabaseModelFactoryTest(GaussDBDatabaseModelFixture fixture)
    {
        Fixture = fixture;
...
""",
            ...
            dbModel =>
            {
                ...
                Assert.Equal(GaussDBResources.LogMissingSchema(new TestLogger<GaussDBLoggingDefinitions>()).EventId, Id);
                Assert.Equal(
                    GaussDBResources.LogMissingSchema(new TestLogger<GaussDBLoggingDefinitions>()).GenerateMessage("MySchema"), Message);
            },
            """
                DROP TABLE "Blank"
                """);
...
""",
            ...
            dbModel =>
            {
                ...
                Assert.Equal(GaussDBResources.LogMissingTable(new TestLogger<GaussDBLoggingDefinitions>()).EventId, Id);
                Assert.Equal(
                    GaussDBResources.LogMissingTable(new TestLogger<GaussDBLoggingDefinitions>()).GenerateMessage("MyTable"), Message);
            },
            """
                DROP TABLE "Blank"
                """);
...
""",
            ...
            _ =>
            {
                ...
                Assert.Equal(GaussDBResources.LogPrincipalTableNotInSelectionSet(new TestLogger<GaussDBLoggingDefinitions>()).EventId, Id);
                Assert.Equal(
                    GaussDBResources.LogPrincipalTableNotInSelectionSet(new TestLogger<GaussDBLoggingDefinitions>()).GenerateMessage(
            ...
            dbModel =>
            {
                ...
                foreach (var column in dbModel.Tables.Select(t => t.Columns.Single()))
                {
                    ...
                    Assert.Equal(
                        ...
                        (GaussDBValueGenerationStrategy?)column[GaussDBAnnotationNames.ValueGenerationStrategy]);
                }
            },
            """
...
""",
            ...
            dbModel =>
            {
                ...
                // GaussDB has special detection for serial columns (scaffolding them with ValueGenerated.OnAdd
...
""",
            ...
            dbModel =>
            {
                ...
                Assert.Equal(
                    GaussDBValueGenerationStrategy.IdentityAlwaysColumn,
                    (GaussDBValueGenerationStrategy?)idIdentityAlways[GaussDBAnnotationNames.ValueGenerationStrategy]);
                ...
                Assert.Equal(
                    GaussDBValueGenerationStrategy.IdentityAlwaysColumn,
                    (GaussDBValueGenerationStrategy?)identityAlways[GaussDBAnnotationNames.ValueGenerationStrategy]);
                ...
                Assert.Equal(
                    ...
                    (GaussDBValueGenerationStrategy?)identityByDefault[GaussDBAnnotationNames.ValueGenerationStrategy]);
            },
            "DROP TABLE identity");
...
""",
            ...
            dbModel =>
            {
                ...
                Assert.Equal(
                    GaussDBValueGenerationStrategy.IdentityByDefaultColumn,
                    (GaussDBValueGenerationStrategy?)withOptions[GaussDBAnnotationNames.ValueGenerationStrategy]);
                Assert.Equal(
                    new IdentitySequenceOptionsData
                    {
                        ...
                        IsCyclic = true,
                        NumbersToCache = 10
                    }.Serialize(), withOptions[GaussDBAnnotationNames.IdentityOptions]);
                ...
                Assert.Equal(
                    GaussDBValueGenerationStrategy.IdentityByDefaultColumn,
                    (GaussDBValueGenerationStrategy?)withoutOptions[GaussDBAnnotationNames.ValueGenerationStrategy]);
                Assert.Null(withoutOptions[GaussDBAnnotationNames.IdentityOptions]);
                ...
                Assert.Equal(
                    GaussDBValueGenerationStrategy.IdentityByDefaultColumn,
                    (GaussDBValueGenerationStrategy?)bigintWithoutOptions[GaussDBAnnotationNames.ValueGenerationStrategy]);
                Assert.Null(bigintWithoutOptions[GaussDBAnnotationNames.IdentityOptions]);
                ...
                Assert.Equal(
                    GaussDBValueGenerationStrategy.IdentityByDefaultColumn,
                    (GaussDBValueGenerationStrategy?)smallintWithoutOptions[GaussDBAnnotationNames.ValueGenerationStrategy]);
                Assert.Null(smallintWithoutOptions[GaussDBAnnotationNames.IdentityOptions]);
            },
            "DROP TABLE identity");
...
""",
            ...
            dbModel =>
            {
                ...
                Assert.Equal("hash", methodIndex[GaussDBAnnotationNames.IndexMethod]);

                // It's cleaner to always output the index method on the database model,
                // even when it's btree (the default);
                // GaussDBAnnotationCodeGenerator can then omit it as by-convention.
                ...
                Assert.Null(noMethodIndex.FindAnnotation(GaussDBAnnotationNames.IndexMethod));
                //Assert.Equal("btree", noMethodIndex.FindAnnotation(GaussDBAnnotationNames.IndexMethod).Value);
            },
            """
...
""",
            ...
            dbModel =>
            {
                ...
                Assert.Equal(new[] { null, "varchar_pattern_ops" }, indexWith[GaussDBAnnotationNames.IndexOperators]);

                var indexWithout = table.Indexes.Single(i => i.Name == "ix_without");
                Assert.Null(indexWithout.FindAnnotation(GaussDBAnnotationNames.IndexOperators));
            },
            """
...
""",
            ...
            dbModel =>
            {
                ...
                Assert.Equal(
                    ...
                    indexWith[GaussDBAnnotationNames.IndexNullSortOrder]);

                var indexWithout = table.Indexes.Single(i => i.Name == "ix_without");
                Assert.Null(indexWithout.FindAnnotation(GaussDBAnnotationNames.IndexNullSortOrder));
            },
            """
...
""",
            ...
            dbModel =>
            {
                ...
                Assert.Null(indexWith.FindAnnotation(GaussDBAnnotationNames.IndexInclude));
                // Assert.Equal(new[] { "b", "c" }, indexWith.FindAnnotation(GaussDBAnnotationNames.IndexInclude).Value);

                var indexWithout = table.Indexes.Single(i => i.Name == "ix_without");
                Assert.Equal(new[] { "a", "b", "c" }, indexWithout.Columns.Select(i => i.Name).ToArray());
                Assert.Null(indexWithout.FindAnnotation(GaussDBAnnotationNames.IndexInclude));
            },
            """
...
""",
        ...
        string? cleanupSql)
    {
        ...
        try
        {
            var databaseModelFactory = new GaussDBDatabaseModelFactory(
    ...
    public class GaussDBDatabaseModelFixture : SharedStoreFixtureBase<PoolableDbContext>
    {
        protected override string StoreName { get; } = nameof(GaussDBDatabaseModelFactoryTest);

        protected override ITestStoreFactory TestStoreFactory
            => GaussDBTestStoreFactory.Instance;

        public new GaussDBTestStore TestStore
            => (GaussDBTestStore)base.TestStore;
